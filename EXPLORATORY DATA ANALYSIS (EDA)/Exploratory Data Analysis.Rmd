...
- **`Country.Name`**: Nombre del país o región.  
- **`Country.Code`**: Código de tres letras asignado a cada país.  
- **`Indicator.Name`**: Nombre del indicador (ej. emisiones de CO₂, acceso a electricidad).  
- **`Indicator.Code`**: Código asociado al indicador.  
- **`X1960` – `X2020`**: Valores anuales del indicador correspondiente para cada país.  
- **`X2050`**: Proyecciones para 2050 (si están disponibles).  
- **`X`**: Columna vacía (posiblemente residual de la extracción de datos).  
...
- Algunas variables tienen un número significativo de valores faltantes (`NA`).  
- En los primeros años (1960-1980), hay muchas celdas sin datos, probablemente porque los indicadores no estaban disponibles para todos los países en ese período.
...
resultados_parte1 <- resultados[, c("Variable", "Total", "No_Missings", "Missings", "Pct_Missings", "Promedio", "Desv_Std", "Varianza")]
...
resultados_parte1 %>%
  kable(caption = "Análisis univariado de los datos - Parte I", booktabs = TRUE) %>%
  kable_styling(latex_options = c("striped", "hold_position"))

resultados_parte2 %>%
  kable(caption = "Análisis univariado de los datos - Parte II", booktabs = TRUE) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
